...
void multiplica_matrizes(int **a,int **b,int **c,int nla,int nca, int ncb)
{
    ...
    for(int i=0;i<nla;i++)
    {
        ...
        {
            aux = 0;
            // laço responsável pelo produto de cada elemento da linha pelo respectivo elemento da coluna fazendo as respectivas somas
            for(int x=0;x<nca;x++) // laço responsavel pelo produto de cada elemento da linha por coluna 
            {
                aux +=  a[i][x] * b[x][j];
            }
            c[i][j] = aux; // guarda na matriz c o resultado da operação no laço anterior
...
int main()
{
    ...
    // aloca a matriz a e seu vetor auxiliar 
    ...
    // chamada da função para fazer a operação 
